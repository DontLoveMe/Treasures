...
-(void)countDownWithTimeStamp:(long long)countDownTime  completeBlock:(void (^)(NSInteger hour,NSInteger minute,NSInteger second,NSInteger millisecond))completeBlock {
    if (_timer==nil) {
        
        __block long long timeout = countDownTime*1000; //倒计时时间
        if (timeout!=0) {
            dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
            _timer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0,queue);
            dispatch_source_set_timer(_timer,dispatch_walltime(NULL, 0),1.0*NSEC_PER_MSEC, 0); //每豪秒执行
            dispatch_source_set_event_handler(_timer, ^{
                if(timeout<0){ //倒计时结束，关闭
                    ...
                    long long hours = timeout/1000/3600;
                    long long minutes = (timeout/1000-hours*3600)/60;
                    long long seconds = timeout/1000-hours*3600-minutes*60;
                    long long millisecond = timeout-(hours*3600+minutes*60+seconds)*1000;
                    dispatch_async(dispatch_get_main_queue(), ^{
                        completeBlock(hours,minutes,seconds,millisecond);
                    });
                    timeout--;
                }
            });
